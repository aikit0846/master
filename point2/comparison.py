...
def make_phase_diagram(pathflow_fname, contour_fname, contour_obj_fname, dirnums, savename, poi, cnum):
    ...
    ax.text(df_flow[poi[0]].iloc[0] + 0.2, df_flow[poi[1]].iloc[0], df_flow[poi[2]].iloc[0], 'α_e^(stay)=0', ha='right')
    ax.text(df_flow[poi[0]].iloc[4] + 0.2, df_flow[poi[1]].iloc[4], df_flow[poi[2]].iloc[4], 'α_e^(stay)=0.3 (Symmetric Case)', ha='left')
    ax.text(df_flow[poi[0]].iloc[len(df_flow) - 1] + 0.2, df_flow[poi[1]].iloc[len(df_flow) - 1], df_flow[poi[2]].iloc[len(df_flow) - 1], 'α_e^(stay)=1', ha='right')
    ...
    colorbar.set_label('Value of objective function when α_e^(stay) = 0.3', rotation=270, va='baseline')

    ## 軸の設定
    ax.set_tlabel('f_(ψ_{})'.format(poi[0]))
    ax.set_llabel('f_(ψ_{})'.format(poi[1]))
    ax.set_rlabel('f_(ψ_{})'.format(poi[2]))
...
### 相図上で解軌道を比較する
def make_trajectory(pathflow_fname, perturbation_fname, contour_fname, contour_obj_fname, dirnums, savename, poi, cnum, arrowpos):
    ## データの読み込み
    # 相図のベースのためのデータを読み込む・データ整形
    df_contour = pd.read_csv(DIR + BASE_SUBDIR + contour_fname)
    df_obj = pd.read_csv(DIR + BASE_SUBDIR + contour_obj_fname)
    df_contour = df_contour[poi]

    # シナリオごとのフローの遷移を読み込む・データ整形
    df_flow_out = pd.read_csv(DIR + BASE_SUBDIR + 'out' + pathflow_fname)
    df_pert_out = pd.read_csv(DIR + BASE_SUBDIR + 'out' + perturbation_fname)
    df_flow_out = df_flow_out[poi]
    df_pert_out = df_pert_out[poi]
    df_flows = [df_flow_out]
    df_perts = [df_pert_out]
    for dirnum in dirnums:
        df_flow_tmp = pd.read_csv(DIR + str(dirnum) + '/' + pathflow_fname)
        df_pert_tmp = pd.read_csv(DIR + str(dirnum) + '/' + perturbation_fname)
        df_flow_tmp = df_flow_tmp[poi]
        df_pert_tmp = df_pert_tmp[poi]
        df_flows.append(df_flow_tmp)
        df_perts.append(df_pert_tmp)
    ...
    ## グリッドと均衡点のプロット
    ax.scatter(df_flows[0][poi[0]].iloc[len(df_flows[0]) - 1], df_flows[0][poi[1]].iloc[len(df_flows[0]) - 1], df_flows[0][poi[2]].iloc[len(df_flows[0]) - 1], color='red', s=100)
    ax.grid()
    ...
    for i in range(len(df_flows)):
        ...
        ax.plot(df_flow[poi[0]], df_flow[poi[1]], df_flow[poi[2]], color='black', lw=1)
        
        for pos in arrowpos:
            length = math.sqrt(df_pert[poi[0]].iloc[pos] ** 2 + df_pert[poi[1]].iloc[pos] ** 2 + df_pert[poi[2]].iloc[pos] ** 2)
            pert0 = df_pert[poi[0]].iloc[pos] * 0.4 / length
            pert1 = df_pert[poi[1]].iloc[pos] * 0.4 / length
            pert2 = df_pert[poi[2]].iloc[pos] * 0.4 / length
            ax.quiver(df_flow[poi[0]].iloc[pos], df_flow[poi[1]].iloc[pos], df_flow[poi[2]].iloc[pos], pert0, pert1, pert2, scale=2, width=0.005, color='black')

    ## 等高線の凡例
    cax = ax.inset_axes([1.05, 0.1, 0.05, 0.9], transform=ax.transAxes)
    colorbar = fig.colorbar(cs, cax=cax)
    colorbar.set_label('Value of objective function', rotation=270, va='baseline')
...
def make_accum_graph(fname_obj, fname_rail, fname_stay, dirnums, savename):
    ...
    ax.set_xlabel('Stay Utility Parameter α_e^(stay)', fontsize=FONTSIZE)
    ...
    ax.set_xticklabels(['Without Control', 'Without Point', '0', '0.1', '0.2', '0.3\n(Symmetric Case)', '0.4', '0.5', '0.6', '0.7', '0.8', '0.9', '1'], rotation=45)
...
# make_trajectory(
#     pathflow_fname='pathFlowTrans.csv',
#     perturbation_fname='perturbation.csv',
#     contour_fname='contour.csv',
#     contour_obj_fname='contourObj.csv',
#     dirnums=[92, 93, 94, 95, 96, 97, 98],
#     savename='2_trajectory.png',
#     poi=['0', '1', '2'],
#     cnum=30,
#     arrowpos=[1, 10, 100, 1000, 3000]
# )
