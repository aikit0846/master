...
struct Link {
    ...
    int oidx;  // 始点ノードのidx
    int didx;  // 終点ノードのidx
    char type;  // タイプ（r：鉄道リンク，s：滞在リンク，a：吸収・自宅滞在リンク，o：その他のリンク）
    double x = 0;  // フロー
    ...
    // リンクコストを計算
    void CalcRailLinkCost(std::function<double(double, double)> LinkCostFunc);

    // コスト関数の微分値を計算
    void CalcRailLinkCostPrime(std::function<double(double, double)> LinkCostFuncPrime);
