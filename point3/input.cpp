...
// リンクファイル読み込み用関数
std::vector<Link> ReadLinkFile(std::string linkFName, std::vector<Node> nodeVec)
{
    ...
    for (auto &&link : linkVec) {
        int spatialO, spatialD, time;
        int spatialO2, spatialD2, time2;
        if (link.type != 'i') {
            continue;
        }
        spatialO = nodeVec[link.oidx].i;
        spatialD = nodeVec[link.didx].i;
        time = nodeVec[link.oidx].t;
        for (auto &&link2 : linkVec) {
            if (link2.type != 'i' || link2.idx <= link.idx) {
                continue;
            }
            spatialO2 = nodeVec[link2.oidx].i;
            spatialD2 = nodeVec[link2.didx].i;
            time2 = nodeVec[link2.oidx].t;
            if (time == time2 && (spatialO == spatialO2 || spatialO == spatialD2 || spatialD == spatialO2 || spatialD == spatialD2)) {
                link.pairIdx.emplace_back(link2.idx);
                link2.pairIdx.emplace_back(link.idx);
            }
        }
        link.pairIdx.erase(std::unique(link.pairIdx.begin(), link.pairIdx.end()), link.pairIdx.end());
    }
